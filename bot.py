...
def calculate_numerology(day: int, month: int, year: int) -> str:
    ...
    n1 = sum(digits)  # Первое рабочее число

    # Второе рабочее число
    s = str(n1)
    n2_raw = int(s[0]) + int(s[1]) if len(s) == 2 else int(s[0])
    n2 = n2_raw  # Оставляем 10, 11, 12 как есть

    # Третье рабочее число: n1 - первая цифра дня * 2
    first_day_digit = day // 10 if day >= 10 else day
    n3 = n1 - first_day_digit * 2

    # Четвёртое рабочее число
    s3 = str(n3)
    if len(s3) == 2:
        n4_raw = int(s3[0]) + int(s3[1])
    else:
        n4_raw = int(s3[0])
    n4 = n4_raw  # Оставляем 10, 11, 12 как есть

    return f"{day:02d}.{month:02d}.{year} → {n1}{n2} {n3}{n4}"
